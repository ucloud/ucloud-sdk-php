...
use UCloud\UCDN\Apis\GetNewUcdnDomainHttpCodeRequest;
use UCloud\UCDN\Apis\GetNewUcdnDomainHttpCodeResponse;
use UCloud\UCDN\Apis\GetNewUcdnDomainHttpCodeV2Request;
use UCloud\UCDN\Apis\GetNewUcdnDomainHttpCodeV2Response;
...
use UCloud\UCDN\Apis\GetUcdnDomainRequestNumV2Request;
use UCloud\UCDN\Apis\GetUcdnDomainRequestNumV2Response;
use UCloud\UCDN\Apis\GetUcdnDomainRequestNumV3Request;
use UCloud\UCDN\Apis\GetUcdnDomainRequestNumV3Response;
use UCloud\UCDN\Apis\GetUcdnDomainTrafficRequest;
use UCloud\UCDN\Apis\GetUcdnDomainTrafficResponse;
use UCloud\UCDN\Apis\GetUcdnPassBandwidthRequest;
use UCloud\UCDN\Apis\GetUcdnPassBandwidthResponse;
...
use UCloud\UCDN\Apis\GetUcdnTrafficRequest;
use UCloud\UCDN\Apis\GetUcdnTrafficResponse;
...
class UCDNClient extends Client
{
     ...
     *     "Type" => (integer) 时间粒度（0表示按照5分钟粒度，1表示按照1小时粒度，2表示按照一天的粒度）
     ...
     *
     * $outputs = [
     *     "BandwidthList" => (array<object>) 带宽信息列表，参见BandwidthInfo[
     *         [
     ...
     *             "Time" => (integer) 带宽获取的时间点。格式：时间戳
     *             "CdnRequest" => (number) 返回值返回指定时间区间内的cdn收到的请求次数之和
     *             "OriginRequest" => (number) 返回值返回指定时间区间内的cdn回源的请求次数之和
     *         ]
     *     ]
     * ]
     *
     ...
     * $args = [
     *     "ProjectId" => (string) 项目ID。不填写为默认项目，子帐号必须填写。 请参考[GetProjectList接口](https://docs.ucloud.cn/api/summary/get_project_list)
     *     "AccountType" => (string) 指定按项目查询，还是按整个账户查询  取值 top 表示按整个账户查询，取值org表示按项目查询
     *     "DomainId" => (array<string>) 域名ID，创建加速域名时生成，n从自然数0开始。默认全部域名
     *     "Areacode" => (string) 查询流量区域 cn代表国内 abroad代表海外，默认全部区域
     *     "BeginTime" => (integer) 查询的起始日期，格式为Unix Timestamp。如果有EndTime，BeginTime必须赋值
     *     "EndTime" => (integer) 查询的结束日期，格式为Unix Timestamp。EndTime默认为当前时间，BeginTime默认为当前时间前一天
     * ]
     *
     * Outputs:
     *
     * $outputs = [
     *     "TrafficSet" => (array<object>) 流量实例表，具体结构见 UcdnDomainTrafficSet[
     *         [
     *             "Time" => (integer) 流量获取的时间点，格式为Unix Timestamp
     *             "Value" => (number) 查询每日流量总值，单位：GB
     *         ]
     *     ]
     * ]
     *
     ...
     *
     * $outputs = [
     *     "BandwidthDetail" => (array<object>) 回源带宽数据[
     *         [
     ...
     * $args = [
     *     "ProjectId" => (string) 项目ID。不填写为默认项目，子帐号必须填写。 请参考[GetProjectList接口](https://docs.ucloud.cn/api/summary/get_project_list)
     *     "Type" => (string) 刷新类型，file代表文件刷新，dir 代表路径刷新
