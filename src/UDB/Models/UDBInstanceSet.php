...
use UCloud\UDB\Models\DescribeUDBInstanceResponse;
use UCloud\UDB\Models\DescribeMongoDBShardedClusterResponse;
...
class UDBInstanceSet extends Response
{
     ...
     *
     * @return UDBSlaveInstanceSetModel[]|null
     */
    public function getDataSet()
    {
        ...
        foreach ($items as $i => $item) {
            array_push($result, new UDBSlaveInstanceSetModel($item));
        }
        return $result;
    }
     ...
     * DataSet: 如果在需要返回从库的场景下，返回该DB实例的所有从库DB实例信息列表。列表中每一个元素的内容同UDBSlaveInstanceSet 。如果这个DB实例没有从库的情况下，此时返回一个空的列表
     *
     * @param UDBSlaveInstanceSetModel[] $dataSet
     */
     ...
     *
     * @return UFileDataSetModel|null
     */
    public function getUserUFileData()
    {
        return new UFileDataSetModel($this->get("UserUFileData"));
    }

    /**
     * UserUFileData: 用户转存备份到自己的UFILE配置, 结构参考UFileDataSet
     *
     * @param UFileDataSetModel $userUFileData
     */
    public function setUserUFileData(UFileDataSetModel $userUFileData)
    {
